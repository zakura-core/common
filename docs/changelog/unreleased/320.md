...
- Reduced cold four-Action Orchard proof generation by 0.475 ms (0.34%) in a
  ...
  evaluation. A one-Action benchmark showed no regression, and a prior
  six-worker AMD Linux benchmark confirmed the optimization is portable.
  Verifier zero checks retain their paired-window schedule; proof bytes and
