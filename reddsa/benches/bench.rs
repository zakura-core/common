...
fn orchard_sigs_with_distinct_keys() -> impl Iterator<Item = OrchardItem> {
    std::iter::repeat_with(|| {
        let mut rng = thread_rng();
        let msg = b"Bench";
        match rng.random::<u8>() % 2 {
            0 => {
                let sk = SigningKey::<orchard::SpendAuth>::new(thread_rng());
                let vk_bytes = VerificationKey::from(&sk).into();
                let sig = sk.sign(thread_rng(), &msg[..]);
                OrchardItem::SpendAuth { vk_bytes, sig }
            }
            1 => {
                let sk = SigningKey::<orchard::Binding>::new(thread_rng());
                let vk_bytes = VerificationKey::from(&sk).into();
                let sig = sk.sign(thread_rng(), &msg[..]);
                OrchardItem::Binding { vk_bytes, sig }
            }
            _ => panic!(),
        }
    })
...
fn bench_batch_verify_orchard(c: &mut Criterion) {
    let mut group = c.benchmark_group("RedPallas Batch Verification");
    for &n in [1usize, 8, 16, 32, 64].iter() {
        group.throughput(Throughput::Elements(n as u64));

        let sigs = orchard_sigs_with_distinct_keys()
            .take(n)
            .collect::<Vec<_>>();
...
criterion_group!(benches, bench_batch_verify, bench_batch_verify_orchard);
