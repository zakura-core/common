...
impl<'a, 'b> Sub<&'b Fp> for &'a Fp {
    ...
    fn sub(self, rhs: &'b Fp) -> Fp {
        ...
        {
            Fp(super::x86_64_asm::sub(&self.0, &rhs.0, &X86_64_ASM_PARAMS))
        }
        #[cfg(not(any(
            all(
                feature = "aarch64-asm",
...
impl<'a, 'b> Add<&'b Fp> for &'a Fp {
    ...
    fn add(self, rhs: &'b Fp) -> Fp {
        ...
        {
            Fp(super::x86_64_asm::add(&self.0, &rhs.0, &X86_64_ASM_PARAMS))
        }
        #[cfg(not(any(
            all(
                feature = "aarch64-asm",
...
fn asm_arithmetic_canonical_sweep_matches_portable() {
    ...
    for _ in 0..200_000u32 {
        ...
        assert_eq!(add, Fp::add(&a, &b), "lhs {:x?} rhs {:x?}", a.0, b.0);
        assert_eq!(sub, Fp::sub(&a, &b), "lhs {:x?} rhs {:x?}", a.0, b.0);
        assert_eq!(mul, Fp::mul(&a, &b), "lhs {:x?} rhs {:x?}", a.0, b.0);
