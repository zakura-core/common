...
//! 0.472 ms (95% CI 0.299--0.646 ms) and four-Action proofs by 0.987 ms
//! (95% CI 0.678--1.290 ms). Explicit preparation added about 30.0 ms. Its
//! two prepared tables account for about 49.6 MiB instead of 24.8 MiB. Other
//! targets, non-power-of-two sizes, and the public orbit-enabled planner
//! retain the 13 MiB policy.
//!
...
/// account for about 49.6 MiB in total, plus small metadata and allocator
/// overhead.
...
impl<C: GlvParams> PreparedZeroMsm<C> {
    ...
    /// α8 for measured power-of-two inputs on AArch64 macOS when it fits the
...
fn plan_no_orbits_mode<C: GlvParams>(terms: usize, num_threads: usize) -> Option<CodebookMode> {
    ...
    if default_mode != ALPHA_SEVEN.mode || !NO_ORBITS_ALPHA_EIGHT_TARGET || !terms.is_power_of_two()
    {
...
mod tests {
    ...
    fn no_orbits_plan_scopes_alpha_eight_to_measured_inputs() {
        for threads in [1usize, 32] {
            ...
            assert_eq!(
                plan_no_orbits_mode::<pallas::Point>(2_048, threads),
                Some(expected),
            );
            let default =
        ...
        for threads in [1usize, 32] {
            assert_eq!(
                plan_no_orbits_mode::<pallas::Point>(600, threads),
                plan_mode::<pallas::Point>(600, threads, DEFAULT_TABLE_FOOTPRINT_BUDGET),
            );
        }
