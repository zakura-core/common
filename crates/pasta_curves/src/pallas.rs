...
/// parallel.
///
/// This helper exposes instruction-level parallelism across the two mixed
/// additions. Each input must be nonidentity, and each projective point must
/// differ from both signs of its affine addend. If these requirements are
/// violated, the returned group result is incorrect.
#[doc(hidden)]
#[inline(always)]
pub fn add_mixed_pair_unchecked(
    lhs_0: &Point,
    rhs_0: &Affine,
    lhs_1: &Point,
    rhs_1: &Affine,
) -> [Point; 2] {
    Point::add_mixed_pair_unchecked(lhs_0, rhs_0, lhs_1, rhs_1)
}

#[test]
fn add_mixed_pair_unchecked_matches_complete_addition() {
    use group::{Curve, Group};
    ...
    assert_eq!(
        add_mixed_pair_unchecked(&point, &other.to_affine(), &ordinary.0, &ordinary.1),
        [point + other, ordinary.0 + ordinary.1],
    );
}
