...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    group.bench_function("mul_chain/64", bench_fq_mul_chain);
    group.bench_function("mul_indep/4", bench_fq_mul_indep_4);
    group.bench_function("mul_indep/8", bench_fq_mul_indep_8);
    group.bench_function("square_chain/64", bench_fq_square_chain);
    group.bench_function("add_chain/64", bench_fq_add_chain);
    group.bench_function("pow_vartime_q_minus_2", bench_fq_pow_vartime_q_minus_2);
}
...
fn bench_fq_mul_chain(b: &mut Bencher) {
    const SAMPLES: usize = 1000;
    ...
    let v: Vec<Fq> = (0..SAMPLES).map(|_| Fq::random(&mut rng)).collect();
    let chain: Vec<Fq> = (0..64).map(|_| Fq::random(&mut rng)).collect();
...
fn bench_fq_mul_indep_4(b: &mut Bencher) {
    const SAMPLES: usize = 1000;
...
fn bench_fq_mul_indep_8(b: &mut Bencher) {
    const SAMPLES: usize = 1000;
...
fn bench_fq_square_chain(b: &mut Bencher) {
    const SAMPLES: usize = 1000;
...
fn bench_fq_add_chain(b: &mut Bencher) {
    const SAMPLES: usize = 1000;
...
/// `pow_vartime` with the Fermat exponent `q - 2`: ~250 squarings fused into
...
/// Fermat inversion.
fn bench_fq_pow_vartime_q_minus_2(b: &mut Bencher) {
    const SAMPLES: usize = 1000;

    /// q - 2, little-endian limbs.
    const EXP: [u64; 4] = [
        0x8c46eb20ffffffff,
        0x224698fc0994a8dd,
        0x0000000000000000,
