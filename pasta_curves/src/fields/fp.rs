...
impl Fp {
    ...
    pub const fn double(&self) -> Fp {
        Fp(portable::double(&self.0, &MODULUS.0))
    }
    ...
    pub const fn sub(&self, rhs: &Self) -> Self {
        Fp(portable::sub(&self.0, &rhs.0, &MODULUS.0))
    }

    /// Adds `rhs` to `self`, returning the result.
    #[cfg_attr(not(feature = "uninline-portable"), inline)]
    pub const fn add(&self, rhs: &Self) -> Self {
        Fp(portable::add(&self.0, &rhs.0, &MODULUS.0))
    }

    /// Negates `self`.
    #[cfg_attr(not(feature = "uninline-portable"), inline)]
    pub const fn neg(&self) -> Self {
        Fp(portable::neg(&self.0, &MODULUS.0))
    }

    /// Multiplies `rhs` by `self`, returning the unreduced 512-bit product.
    #[cfg_attr(not(feature = "uninline-portable"), inline)]
    pub(crate) const fn mul_unreduced(&self, rhs: &Self) -> [u64; 8] {
        portable::mul_wide(&self.0, &rhs.0)
    }
...
/// An `Fp` value below `2p`; see [`super::lazy`].
///
...
impl LazyElement<Fp> for FpLazy {
    ...
    fn mul(&self, rhs: &Fp) -> FpLazy {
        ...
        ))]
        {
            ...
            FpLazy(Fp(self.0).mul_runtime(rhs).0)
        }
        ...
        {
            FpLazy(portable::mul_lazy(&self.0, &rhs.0, &MODULUS.0, INV))
        }
    }
    ...
    fn square(&self) -> FpLazy {
        ...
        {
            FpLazy(Fp(self.0).square_runtime().0)
        }
        ...
        {
            FpLazy(portable::square_lazy(&self.0, &MODULUS.0, INV))
        }
    }
    ...
    fn add(&self, rhs: &Fp) -> FpLazy {
        ...
        FpLazy(portable::add(&self.0, &rhs.0, &MODULUS.0))
    }
    ...
    fn add_lazy(&self, rhs: &FpLazy) -> FpLazy {
        ...
        {
            FpLazy(Fp(self.0).add(&Fp(rhs.0)).0)
        }
        ...
        {
            FpLazy(portable::add_lazy(&self.0, &rhs.0, &TWO_MODULUS))
        }
    }

    #[inline]
    fn sub(&self, rhs: &Fp) -> FpLazy {
        FpLazy(portable::sub(&self.0, &rhs.0, &MODULUS.0))
    }
    ...
    fn sub_lazy(&self, rhs: &FpLazy) -> FpLazy {
        ...
        {
            FpLazy(Fp(self.0).sub(&Fp(rhs.0)).0)
        }
        ...
        )))]
        {
            ...
            // leaves it in [0, 2p).
            FpLazy(portable::sub(&self.0, &rhs.0, &TWO_MODULUS))
        }
    }
    ...
    fn reduce(self) -> Fp {
        ...
        {
            Fp(portable::canonicalize(&self.0, &MODULUS.0))
        }
    }
}
...
fn lazy_arithmetic_matches_canonical() {
    ...
    let below_two_p = |v: &FpLazy| portable::is_below_twice(&v.0, &MODULUS.0);

    for _ in 0..5_000 {
        let a = <Fp as ff::Field>::random(&mut rng);
        let b = <Fp as ff::Field>::random(&mut rng);
        let c = <Fp as ff::Field>::random(&mut rng);
    ...
    assert_eq!(Fp::zero().lazy().mul(&max).sub(&max).reduce(), -max);
    assert_eq!(max.lazy().sub(&max).reduce(), Fp::zero());
    ...
    for _ in 0..5_000 {
        let x = <Fp as ff::Field>::random(&mut rng);
        let y = <Fp as ff::Field>::random(&mut rng);
        let raw = FpLazy(portable::add(&x.0, &MODULUS.0, &[u64::MAX; 4]));
        assert!(below_two_p(&raw) && !is_canonical(&Fp(raw.0)));
        ...
        let raw_y = FpLazy(portable::add(&y.0, &MODULUS.0, &[u64::MAX; 4]));
        ...
        // 256 bits.
        let top = FpLazy(portable::sub(&TWO_MODULUS, &[1, 0, 0, 0], &[0; 4]));
        assert!(below_two_p(&top));
        assert_eq!(
            top.add_lazy(&top).reduce(),
            Fp(portable::canonicalize(&top.0, &MODULUS.0)).double()
        );
        assert!(below_two_p(&top.add_lazy(&top)));
        assert_eq!(top.sub_lazy(&top).reduce(), Fp::zero());
    }
}
    ...
    for i in 0..51 {
        ...
        for n in 1..=2048u32 {
            ...
            if n.is_power_of_two() {
                assert_eq!(
                    Fp(super::portable::canonicalize(&acc, &MODULUS.0)),
