...
static_assertions::assert_eq_size!(Fq, [u64; 4]);
static_assertions::assert_eq_align!(Fq, u64);
...
impl Fq {
    ...
    const fn montgomery_reduce(
        ...
        (&Fq([r4, r5, r6, r7])).sub(&Fq(MODULUS_LIMBS))
    }
    ...
    fn mul_runtime(&self, rhs: &Self) -> Self {
        ...
        {
            Fq(super::aarch64_asm::mul(&self.0, &rhs.0, &MODULUS_LIMBS, INV))
        }
    ...
    fn square_runtime(&self) -> Self {
        ...
        {
            Fq(super::aarch64_asm::square(&self.0, &MODULUS_LIMBS, INV))
        }
...
fn aarch64_asm_matches_portable_arithmetic() {
    ...
    let max_montgomery_residue = Fq([MODULUS_LIMBS[0] - 1, MODULUS_LIMBS[1], MODULUS_LIMBS[2], MODULUS_LIMBS[3]]);
    let boundaries = [
        Fq::zero(),
        Fq::one(),
        -Fq::one(),
