...
impl Fq {
    ...
    pub const fn double(&self) -> Fq {
        Fq(portable::double(&self.0, MODULUS.0[0], MODULUS.0[1]))
    }
    ...
    pub const fn square(&self) -> Fq {
        ...
        Fq(portable::montgomery_reduce_low(
    ...
    pub const fn sub(&self, rhs: &Self) -> Self {
        Fq(portable::sub(&self.0, &rhs.0, MODULUS.0[0], MODULUS.0[1]))
    }

    /// Adds `rhs` to `self`, returning the result.
    #[cfg_attr(not(feature = "uninline-portable"), inline)]
    pub const fn add(&self, rhs: &Self) -> Self {
        Fq(portable::add(&self.0, &rhs.0, MODULUS.0[0], MODULUS.0[1]))
    }

    /// Negates `self`.
    #[cfg_attr(not(feature = "uninline-portable"), inline)]
    pub const fn neg(&self) -> Self {
        Fq(portable::neg(&self.0, MODULUS.0[0], MODULUS.0[1]))
    }
...
fn montgomery_reduce_low_matches_classical() {
    ...
    for _ in 0..20_000 {
        ...
        let classical = Fq::montgomery_reduce(t[0], t[1], t[2], t[3], t[4], t[5], t[6], t[7]);
        let low_half = Fq(super::portable::montgomery_reduce_low(
            &t,
            MODULUS.0[0],
            MODULUS.0[1],
            INV,
    ...
    for t in [
        ...
        assert_eq!(
            Fq::montgomery_reduce(t[0], t[1], t[2], t[3], t[4], t[5], t[6], t[7]),
            Fq(super::portable::montgomery_reduce_low(
                &t,
                MODULUS.0[0],
                MODULUS.0[1],
                INV
