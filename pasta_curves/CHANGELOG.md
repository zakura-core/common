...
- `Fp`/`Fq` field inversion is now a variable-time 62-divstep safegcd (a
  ...
  The cheaper inversion re-tunes the GLV batch-affine threshold
  `BATCH_AFFINE_MIN_POINTS` from 512 down to 64 live points (its measured
  break-even; −5% to −8% per point at 128–256 versus the per-point ladders).
...
- Added `glv::Table::mul_decomposed_batch`, which multiplies many points by one
  ...
  64 live points, and the scalar-dependent exceptional schedules (checked exactly
